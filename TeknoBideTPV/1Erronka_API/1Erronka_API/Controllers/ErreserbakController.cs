...
namespace _1Erronka_API.Controllers
{
    ...
    public class ErreserbakController : ControllerBase
    {
        ...
        public IActionResult DeskargatuTicket(int id)
        {
            ...
            var fileName = Path.GetFileName(erreserba.FakturaRuta);
            string rutaAbsoluta = Path.Combine(Directory.GetCurrentDirectory(), "storage", "tiketak", fileName);

            if (!System.IO.File.Exists(rutaAbsoluta))
            {
                rutaAbsoluta = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", erreserba.FakturaRuta.TrimStart('/'));
            }
